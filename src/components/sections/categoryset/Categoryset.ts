...
export default function CategorySet(DOM: HTMLDivElement, posts: SliderType) {

    const openModal = (id: string) => {
        const selectedItem = posts.find((item) => item._id === id);
        if (selectedItem) {
    ...
    const filterByCategory = (category: string) => {
        return posts.filter((item) => item.collection === category);
    };
